...
async function loadMeetings(meetingListId) {
  ...
  const response = await apiGet(
    apiConfigWithSite.uri + 'lists/' + meetingListId + '/items?$expand=fields' + filterString,
  );
  if (response.success) {
...
async function patchMeeting(meetingId, meetingTitle, processedReports) {
  ...
  try {
    const path =
      apiConfigWithSite.uri + 'lists/' + configuration.MeetingListId + '/items/' + meetingId;
    ...
    if (response.success) {
      ...
      if (!meetingFields.Processed || meetingFields.Processedreports != processedReportsText) {
        const response = await apiPatch(path, {
          fields: {
