...
async function processFlows(configuration) {
  try {
    ...
    const countries = await getCountries(configuration),
      spFlows = await loadItems(configuration.ReportnetFlowsListId),
...
async function getCountries(configuration) {
  const response = await apiGet(
    apiConfigWithSite.uri + '/lists/' + configuration.UserListId + '/columns',
  );
...
function mapFlows(configuration, flows, spFlows, spObligations) {
  return flows.map((flow) => {
    ...
    return {
      ...
      ...(spObligation && { IsEEACore: spObligation.fields.IsEEACore }),
...
async function saveFlow(configuration, flow) {
  let path = `${apiConfigWithSite.uri}lists/${configuration.ReportnetFlowsListId}/items`;
  if (flow.id) {
    path += `/${flow.id}`;
    delete flow.id;
