const logging = require('../../lib/logging'),
  ...
  { apiConfig } = require('../../lib/graphClient');
...
async function addTag(teamId, name, userId, email) {
  let response = await apiGet(
    `${apiConfig.uri}/teams/${teamId}/tags?$filter=displayName eq '${name}'`,
  );
  ...
  if (response?.success && response?.data?.value?.length) {
    ...
    if (!tagMemberIdResponse?.data?.value?.length) {
      postResponse = await apiPost(
        `${apiConfig.uri}/teams/${teamId}/tags/${existingTag.id}/members`,
        {
          userId: userId,
        },
      );
